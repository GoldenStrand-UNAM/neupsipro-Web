const request = require('supertest');

// ===================== MOCKS =============================================

let mockAuthBehavior = 'authenticated';

jest.mock('../../../Back/src/infrastructure/auth/auth.middleware', () =>
  jest.fn(() => ({
    verifyToken: (req, res, next) => {
      if (mockAuthBehavior === 'unauthenticated') {
        return res.redirect('/auth/');
      }
      req.user = { id: 1, role: 'admin' };
      next();
    },
  }))
);

jest.mock('../../../Back/src/infrastructure/auth/permissions.middleware', () =>
  jest.fn(() => ({
    requirePermission: () => (req, res, next) => next(),
  }))
);

jest.mock('../../../Back/src/infrastructure/external/rateLimiting', () =>
  () => (_req, _res, next) => next()
);

const mockExecuteMoca = jest.fn();

jest.mock('../../../Back/src/application/usecase/testApplications/postMOCAUseCase', () =>
  jest.fn().mockImplementation(() => ({ execute: mockExecuteMoca }))
);

const app = require('../../../Back/src/app');

afterAll(async () => { await app.close?.(); });

// ===================== HELPERS ===========================================

const asAuthenticated   = () => { mockAuthBehavior = 'authenticated'; };
const asUnauthenticated = () => { mockAuthBehavior = 'unauthenticated'; };

const url = '/api/users/1/applications/app-001/tests/4/results';

const validBody = () => ({ score: 27, notes: 'Rendimiento normal.' });

const successDTO = {
  idResults: 'r-004', idTest: 4, status: 3,
  score: 28, interpretation: 'Rendimiento cognitivo normal', notes: 'OK',
};

// ===================== TESTS =============================================

describe('INTEGRATION — POST /api/users/:id_user/applications/:id_application/tests/4/results · MOCA (R01 / R04)', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    asAuthenticated();
  });

  test('base — responds 2xx with valid score (Registrar)', async () => {
    mockExecuteMoca.mockResolvedValue(successDTO);

    const res = await request(app).post(url).send(validBody());

    expect([200, 201]).toContain(res.status);
    expect(mockExecuteMoca).toHaveBeenCalledTimes(1);
  });

  test('base — responds 2xx when re-saving with new score (Modificar)', async () => {
    mockExecuteMoca.mockResolvedValue(successDTO);

    const res = await request(app)
      .post(url)
      .send({ score: 25, notes: 'Re-evaluado tras observación.' });

    expect([200, 201]).toContain(res.status);
  });

  // 1.1 — SQL passes through as plain string; parameterised queries prevent injection.
  test('1.1 accepts SQL injection in notes as plain string', async () => {
    mockExecuteMoca.mockResolvedValue(successDTO);

    const res = await request(app)
      .post(url)
      .send({ ...validBody(), notes: "'; DROP TABLE moca_results; --" });

    expect([200, 201]).toContain(res.status);
    expect(res.text).not.toContain('DROP');
    expect(res.text).not.toContain('stack');
  });

  // 1.2 — XSS passes through as plain string; neutralised at render time via EJS escaping.
  test('1.2 accepts an XSS payload in notes as plain string', async () => {
    mockExecuteMoca.mockResolvedValue(successDTO);

    const res = await request(app)
      .post(url)
      .send({ ...validBody(), notes: '<script>alert(document.cookie)</script>' });

    expect([200, 201]).toContain(res.status);
    expect(res.text).not.toContain('<script>');
    expect(res.text).not.toContain('stack');
  });

  // 1.3 — Emojis: accepted (200/201) or sanitised (400).
  test('1.3 handles emojis in notes without internal errors', async () => {
    mockExecuteMoca.mockResolvedValue(successDTO);

    const res = await request(app)
      .post(url)
      .send({ ...validBody(), notes: 'nawdkjsdbakb 😊🧠✅' });

    expect([200, 201, 400]).toContain(res.status);
    expect(res.text).not.toContain('stack');
  });

  test('responds 422 when score is above 30', async () => {
    const err = new Error('score must be between 0 and 30');
    err.status = 422;
    mockExecuteMoca.mockRejectedValue(err);

    const res = await request(app).post(url).send({ score: 35 });

    expect(res.status).toBe(422);
    expect(res.text).not.toContain('stack');
  });

  test('responds 404 when the result row does not exist', async () => {
    const err = new Error('Test result row not found');
    err.status = 404;
    mockExecuteMoca.mockRejectedValue(err);

    const res = await request(app).post(url).send(validBody());

    expect(res.status).toBe(404);
    expect(res.text).not.toContain('stack');
  });

  test('redirects to /auth/ when there is no active session', async () => {
    asUnauthenticated();

    const res = await request(app).post(url).send(validBody());

    expect(res.status).toBe(302);
    expect(res.header.location).toBe('/auth/');
  });
});