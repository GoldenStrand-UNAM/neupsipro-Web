  ...
  return `
    <div class="modal">
      ...
      <div class="modal__body flex flex-col">
        ...
        ${areaRow('CI Total', { score: test.scoreTotal, interpretation: test.interTotal })}
  ...
  return `
    <div class="modal">
      ...
      <div class="modal__body flex flex-col gap-3">
        ...
        ${areaRow('CI Total', 'inputWAISTotal', 'interpWAISTotal', 'errorWAISTotal', prefill.ciTotal)}
...
function bindWAISFormListeners (idUser, idApplication, closeModal) {
  ...
  totalEl.addEventListener('input', () => {
    ...
    if (totalEl.value.length > 5) totalEl.value = totalEl.value.slice(0, 5);
  ...
  document.getElementById('btnSaveWAIS').addEventListener('click', async () => {
    ...
    if (totalEl.value.trim() === '' || isNaN(totalVal) || totalVal < 0 || totalVal > 300) {
      totalErrEl.textContent = 'Ingresa un valor válido para CI Total (0–300)';
...
async function openWAISModal (idUser, idApplication, test, mode) {
  ...
  const prefill = {
    ...
    ciTotal: {
      score: (isModify || isConsult) ? (fetchedTest.scoreTotal ?? '') : '',
      interp: (isModify || isConsult) ? (fetchedTest.interTotal  ?? '—') : '—',
    },
