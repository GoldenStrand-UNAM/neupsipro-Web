function interpretWAIS(score) {
  ...
  if (n <= 79)  return 'Limítrofe';
  if (n <= 89)  return 'Promedio Bajo';
  if (n <= 109) return 'Promedio';
  if (n <= 119) return 'Promedio Alto';
  if (n <= 129) return 'Superior';
...
function openWAISModal(idUser, idApplication, test, mode) {
  ...
  const prefillInterp = prefillScore !== '' ? interpretWAIS(prefillScore) : '—';
  ...
  `
    <div class="modal">
      ...
      <div class="modal__header">
        <h2 class="modal__title">WAIS — ${titles[mode]}</h2>
        <button id="btnCloseWAIS" class="modal__close" aria-label="Cerrar modal">
          <svg class="modal__close-icon" xmlns="http://www.w3.org/2000/svg"
               fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12"/>
      ...
      <div class="modal__body flex flex-col gap-6">
        ...
        <div class="flex flex-col gap-2">
          ...
          <p id="waisNotesCount" class="text-lg text-gray-400 text-right">0 / 200</p>
  ...
  const scoreInput     = document.getElementById('inputWAISScore');

  const notesInput     = document.getElementById('inputWAIS notes');
  const notesCount     = document.getElementById('waisNotesCount');
  const interpretLabel = document.getElementById('waisInterpretation');
  ...
  if (!isConsult) {
    scoreInput.addEventListener('input', () => {
      ...
      interpretLabel.textContent = interpretWAIS(n);
    });
    ...
    document.getElementById('btnSaveWAIS').addEventListener('click', async () => {
      ...
      const notes = document.getElementById('inputWAIS notes').value.trim() || null;
