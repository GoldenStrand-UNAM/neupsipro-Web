class updateSessionUseCase {
  ...
  async execute (data) {
    const { id_user, id_session, session_date } = data;
    ...
    const updated = await this.interventionRepository.updateSession({
      id_session,
      session_number,
      session_date,
      objectives,
    ...
    if (!updated) throw new Error('No se pudo actualizar la sesión');

    return { success: true, message: 'Sesión actualizada' };
  }
}
module.exports = updateSessionUseCase;